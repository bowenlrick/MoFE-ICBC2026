...
class MoE_GibbsFNO(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.bmm(weights.unsqueeze(1), expert_outputs).squeeze(1), weights
...
def evaluate_market_value(model, X, y, dates, base_prices, scaler, config):
    ...
    pred_cum_ret = np.sum(pred_log_ret, axis=1)
    actual_cum_ret = np.sum(actual_log_ret, axis=1)

    pred_prices = base_prices * np.exp(pred_cum_ret)
    actual_prices = base_prices * np.exp(actual_cum_ret)

    rmse = np.sqrt(mean_squared_error(actual_prices, pred_prices))
    mae = mean_absolute_error(actual_prices, pred_prices)
    r2 = r2_score(actual_prices, pred_prices)

    ic, _ = pearsonr(pred_cum_ret, actual_cum_ret)
    correct_dir = (np.sign(pred_cum_ret) == np.sign(actual_cum_ret))
    win_rate = np.mean(correct_dir)

    pos_actual = actual_cum_ret[correct_dir & (actual_cum_ret > 0)]
    neg_actual = actual_cum_ret[correct_dir & (actual_cum_ret < 0)]
    pl_ratio = (np.mean(pos_actual) / abs(np.mean(neg_actual))) if (len(neg_actual) > 0 and len(pos_actual) > 0) else 0

    threshold = 0.005
    strat_returns = np.where(np.abs(pred_cum_ret) > threshold,
                             np.sign(pred_cum_ret) * actual_cum_ret,
                             0)
    ...
    sharpe = (mean_ret / (std_ret + 1e-9)) * np.sqrt(365.0 / 5.0)  
    ...
    return {
        ...
        'pred_log_ret': pred_cum_ret,  
        'actual_log_ret': actual_cum_ret  
    }
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    plt.savefig('bitcoin_prediction_optimized_final5.png')
    print("[Info] Visualization saved as 'bitcoin_prediction_optimized_final5.png'")